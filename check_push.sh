...
#	Description: 监测指定服务是否存活并推送消息至Telegram
#=================================================
...
CONF="${file_1}/push.conf"
LOG_file="${file_1}/check_push.log"
Test_link="127.0.0.1:81/test/memory
127.0.0.1:82
127.0.0.1:83"
...
check_crontab_monitor_status(){
	...
	cron_config=$(cat ".crontab_tmp" | grep "check_push.sh monitor")
	rm -rf ".crontab_tmp"
...
POST_TG(){
	Get_IP
	if [[ -z "${NAME}" ]]; then
		wget -qO- --post-data="text=\`[服务离线警告]\`  —  \[\`${IP}\`]&parse_mode=Markdown&disable_notification=false"  "https://tgbot.lbyczf.com/sendMessage/${TOKEN}" >> ${LOG_file}
	else
		wget -qO- --post-data="text=\`[服务离线警告]\`  —  \[${NAME}] (\`${IP}\`)&parse_mode=Markdown&disable_notification=false"  "https://tgbot.lbyczf.com/sendMessage/${TOKEN}" >> ${LOG_file}
	fi
	echo "" >> ${LOG_file}
}
Get_IP(){
...
Add_Crontab(){
	...
	sed -i "/check_push.sh monitor/d" "$file_1/crontab.bak"
	echo -e "\n* * * * * /bin/bash $file_1/check_push.sh monitor" >> "$file_1/crontab.bak"
...
Test_SILL(){
	...
	if [[ "${SILL_NOW}" == "${SILL}" ]]; then
		echo -e "${Error} 短时间内检测服务状态结果为未存活已达到阈值 [${SILL}]，将不会继续检测。跳过..." && exit 1
...
crontab_monitor(){
	...
	if [[ "${all_status_num}" == "${Test_total}" ]]; then
		sill_new="0"
		Write_config_now_sill
		echo -e "${Info} ${DATE} 全部服务测试通过！没有服务离线。"| tee -a ${LOG_file}
		...
		echo -e "${Error} ${DATE} 全部服务测试失败！被检测服务疑似离线，累计次数中..."| tee -a ${LOG_file}
		echo -e "${Error} ${DATE} 所有服务离线 ${Test_total} ，开始强行推送三遍..."| tee -a ${LOG_file}
		POST_TG
		sleep 10s
		POST_TG
		sleep 10s
		...
		sill_new="0"
		Write_config_now_sill
		echo -e "${Info} ${DATE} 部分服务测试离线，发起通知。"| tee -a ${LOG_file}
		POST_TG
	fi
}
Init_config(){
	touch check_push.log
	Set_Name
	Set_Token
	Install_crontab
	Add_Crontab
	...
	sill_new="0"
	Write_config
	echo -e "${Info} 初始化配置完成，目前已启动定时检测服务存活状态。"
}
Uninstall_config(){
...
if [[ "${action}" == "monitor" ]]; then
	...
	echo && echo -e "  监测服务是否存活 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
  ---- 选项菜单 ----

 ${Green_font_prefix} 1.${Font_color_suffix} 初始化
 ${Green_font_prefix} 2.${Font_color_suffix} 卸  载
————————————
 ${Green_font_prefix} 3.${Font_color_suffix} 手动 检测服务
 ${Green_font_prefix} 4.${Font_color_suffix} 暂停 监测服务
 ${Green_font_prefix} 5.${Font_color_suffix} 重启 监测服务(或清零阈值)
 —— 当暂停或脚本推送三次服务离线信息后，
    可以用该选项使脚本继续监测服务。
————————————
